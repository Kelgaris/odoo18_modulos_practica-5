...
class LigaEquipo(models.Model):
    ...
    # Declaramos campo para los puntos extra obtenidos por goleada
    puntos_extra = fields.Integer(default=0, store=True, help='Puntos extra obtenidos por diferencia de 4+ goles')
    
    puntos= fields.Integer( compute="_compute_puntos",default=0, store=True)
    
    @api.depends('victorias','empates','puntos_extra')
    def _compute_puntos(self):
        for record in self:
            # Calculamos los puntos base: 3 por victoria, 1 por empate
            # Sumamos los puntos extra obtenidos por goleadas (diferencia >= 4 goles)
            record.puntos = record.victorias * 3 + record.empates + record.puntos_extra
